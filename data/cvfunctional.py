...
def transform_dct_size(img, encoder, size):
    ...
    return dct_y, dct_cb, dct_cr
def upscale(img, upscale_factor, interpolation='BILINEAR'):
    # 判断是否是 Tensor 类型，如果是则转换为 Numpy 数组
    if isinstance(img, torch.Tensor):
        img = img.cpu().numpy()  # 将 Tensor 转为 Numpy 数组
    ...
    # 进行上采样
    upscaled_img = cv2.resize(img, (new_w, new_h), interpolation=INTER_MODE[interpolation])

    # 如果原图是 Tensor 类型，并且希望返回 Tensor 类型，可以将结果转回 Tensor

    upscaled_img = torch.from_numpy(upscaled_img).float()
